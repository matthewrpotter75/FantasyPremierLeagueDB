...
USING 
(
	SELECT 0 AS Id, ph.gameweekId, t.id AS homeTeamId, ot.id AS awayTeamId, t.short_name AS homeTeam_Shortname, ot.short_name AS awayTeam_Shortname, ph.kickoff_time--, COUNT(DISTINCT ph.playerId) AS playerCount
	...
	ON p.teamId = t.id
	WHERE ph.was_home = 1
	GROUP BY ph.gameweekId, t.id, ot.id, t.short_name, ot.short_name, ph.kickoff_time
